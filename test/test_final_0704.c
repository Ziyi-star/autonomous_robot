...
// Function to check if the robot is on the start field for more than 1 second
bool check_start_field_time(int *start_time, uint16_t current_time) {
    if (*start_time == 0) {
        *start_time = current_time;
    }
    
    if (current_time - *start_time > 100) { // Check if more than 1 second has passed
        stop();
        return true; // Indicate that the robot should stop
    }
    
    return false; // Indicate that the robot should continue
}
...
int main(void){
    ...
    while(1){
        ...
        if (left && middle && right){
			if (check_start_field_time(&start, m_second)) {
				start = 0;
                break; // Exit the loop if the robot should stop
            }
        } 

        if (message == 'S' && count_run == 0){
